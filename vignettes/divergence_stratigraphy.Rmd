...
of interest. __Divergence Stratigraphy__ is the process of quantifying the selection pressure (in terms of amino acid sequence divergence) acting on orthologous genes between closely related species. The resulting sequence divergence map (short divergence map), stores the divergence stratum in the first column and the query_id of inferred orthologous genes in the second column ([Quint et al., 2012](http://www.nature.com/nature/journal/v490/n7418/full/nature11394.html); [Drost et al., 2015](http://mbe.oxfordjournals.org/content/32/5/1221.full)).
...
are already included in the `orthologr` package and do not have to be installed separately. Nevertheless, make sure __you have BLAST installed on your machine before using `divergence_stratigraphy()`__.  

__Note__: The following examples assume that the __BLAST__ program is installed in your default execution path `usr/local/bin`. In case you do not have __BLAST__ installed yet or the following command in R produces a different output, please consult the [Installation Vignette](https://github.com/HajkD/orthologr/blob/master/vignettes/Install.Rmd#orthology-inference-tools) to corretly set up the __BLAST__ program to perform __Divergence Stratigraphy__.  
...
In [Drost et al., 2015](http://mbe.oxfordjournals.org/content/32/5/1221.full) we define a `Divergence Map` as table storing the degree of sequence divergence (= `divergence strata`) for each protein coding gene of a given query organism. In this case selection pressure was quantified by dNdS estimation (ratio of synonymous versus non-synonymous `codon -> amino acid` sequence substitution rates).
...
To perform __Divergence Stratigraphy__ using `orthologr` you need the following input files:
...
I implemented the [biomartr](https://github.com/HajkD/biomartr) package to automate the process do sequence data retrieval. The [Sequence Retrieval Vignette](https://github.com/HajkD/biomartr/blob/master/vignettes/Sequence_Retrieval.Rmd) stored in `biomartr` provides detailed use cases for the automation of biological sequence retrieval.

__Note__: Make sure you have [biomartr](https://github.com/HajkD/biomartr#fast-installation-guide) installed before running any `biomartr` functions.
...
The `comp_cores` argument implemented in the `divergence_stratigraphy()` function allows you to specify the number of cores you want to use on your machine. The default value is `comp_cores = 1` which might take __10-12h__ to execute. So make sure you use all cores available on your machine to speed up the computation time.
...
sequences of these organisms. Here the previously downloaded CDS sequence files of _A. thaliana_ (= `query_file`) and _A. lyrata_ (= `subject_file`) need to be specified. The `eval` is set to `1E-5` (default ; see [Quint et al., 2012](http://www.nature.com/nature/journal/v490/n7418/full/nature11394.html)) and `BLAST best reciprocal hit` is used for orthology inference (see [Drost et al., 2015](http://mbe.oxfordjournals.org/content/32/5/1221.full)). In case ``orthologr` is running on a multicore machine, you can set the `comp_cores` argument to any number of cores
supported by your machine. The `clean_folders` argument indicates whether the internal folder structure should be deleted (cleaned) after processing is finished. In this case all output files generated by `divergence_stratigraphy` (stored in `tempdir()`) will be removed after the `Divergence Map` was returned. The `quiet` argument indicates whether a successful interface call should be printed out to the console (`quiet = FALSE`) or not (`quiet = TRUE`).
...
Before running `divergence_stratigraphy()` with two complete genomes, you can first run a test __Divergence Stratigraphy__ with 20 example genes that are stored in the `orthologr` package:
...
To save corresponding `Divergenec Maps` to your hard drive you can
...
This way `write.table()` will store the `Divergence Map` to your current working directory (= `getwd()`).
...
Sometimes the machine you are working on does not allow you to install __BLAST__ in the default execution path `usr/local/bin`. For this purpose the `blast_path` argument is implemented in `divergence_stratigraphy()`. This argument takes an character string specifying the `PATH` to your `blastp` execution file that is stored on a different place than `usr/local/bin`.
...
According to [Drost et al., 2015](http://mbe.oxfordjournals.org/content/32/5/1221.full) the __Divergence Stratigraphy__ algorithm performs __BLAST__ best reciprocal hit (RBH) as orthology inference method. Despite this convention, the `ortho_detection` argument allows you to perform orthology inference within the __Divergence Stratigraphy__ algorithm that is based on any orthology inference method implemented in `orthologr` (see `?orthologs` or [Orthology Inference Vignette](https://github.com/HajkD/orthologr/blob/master/vignettes/orthology_inference.Rmd) for details). For example in [Quint et al., 2012](http://www.nature.com/nature/journal/v490/n7418/full/nature11394.html) instead of __BLAST__ best reciprocal hit, the method __BLAST__ best hit (BH) was used to perform orthology inference within the __Divergence Stratigraphy__ algorithm.
...
`Divergence Maps` can be used for a wide range of analyses. One example is to combine `Divergence Maps` with gene expression data to capture evolutionary signals in developmental transcriptomes (= `Phylotranscriptomics`; see [Drost et al., 2015](http://mbe.oxfordjournals.org/content/32/5/1221.full)). Performing phylotranscriptomic analyses based on an existing `Divergence Map` can easily be done by using the [myTAI](http://cran.r-project.org/web/packages/myTAI/index.html) package. You can consult the [Introduction to the myTAI package Vignette](http://cran.r-project.org/web/packages/myTAI/vignettes/Introduction.html)
for more details.
