...
orthofinder2 <-
        function(proteome_folder,
                ...
                if (!is.null(import_type)) {
                        if (!is.element(import_type,
                                        c("orthogroups_core", "orthogroups_pairwise")))
                                stop(
                                        "The specified 'import_type' is not supported by this function. Please consult the documentation to select a valid Orthofinder2 'import_type'.",
                                        call. = FALSE
                                )
                }
                ...
                if (!file.exists(proteome_folder))
                        stop(
                                "Please provide a valid path to the proteome folder. Your specified path '",
                                proteome_folder,
                                "' seems not to exist.",
                                call. = FALSE
                        )
                ...
                if (any(stringr::str_detect(files, "documentation"))) {
                        message(
                                "Your species folder '",
                                proteome_folder,
                                " still contains the 'documentation' folder which will be moved from ",
                                proteome_folder,
                                " to ",
                                getwd(),
                                " to enable the Orthofinder2 search."
                        )
                        file.rename(
                                file.path(proteome_folder, "documentation"),
                                file.path(getwd(), "documentation")
                        )
                        unlink(
                                file.path(proteome_folder, "documentation"),
                                recursive = TRUE,
                ...
                if (!use_existing_output) {
                        ...
                        message("Running OrthoFinder2 with ",
                                comp_cores,
                                " cores ...")
                        message(
                                "Do your input fasta files store unique sequences per gene locus, e.g. the longest splice variant? If not, then have a look at ?orthologr::retrieve_longest_isoforms()."
                        )
                        ...
                        if (dirname(proteome_folder) == ".") {
                                if (is.null(of_path)) {
                                        ...
                                        message("\n")
                                        message(
                                                "A new path to the orthofinder executable was specified: ",
                                                ws_wrap(
                                                        file.path(of_path, "orthofinder")
                                                ),
                                                " ..."
                                        )
                                        system(
                        ...
                        message("\n")
                        message(
                                "Orthofinder2 finished successfully and stored all results in ",
                                ifelse(
                                        dirname(proteome_folder) == ".",
                                        ws_wrap(file.path(
                                                getwd(), proteome_folder
                                        )),
                                        ws_wrap(proteome_folder)
                                )
                        )
                ...
                orthofinder2_output_folder <-
                        file.path(proteome_folder,
                                  "OrthoFinder",
                                  paste0("Results_", basename(proteome_folder)))
                
                if (!is.null(import_type) &&
                    (import_type == "orthogroups_core")) {
                        res <-
                                orthofinder2_retrieve_core_orthologs(
                                        orthogroups_file = file.path(orthofinder2_output_folder, "") ,
                                        single_copy_file = file.path(orthofinder2_output_folder, "")
                                )
                        return(res)
                }
