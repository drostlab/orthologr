...
extract_features <- function(x, format) {
        ...
        if (format == "gtf") {
                if (!all(c("gene", "CDS") %in% names(table(x$type)))) {
                        stop("Please make sure that the 'type' column in your gtf file are labeled as 'gene' and 'CDS' to denote genes and CDS.", call. = FALSE)
                }
